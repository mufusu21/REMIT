...
class DataPreprocessingReady():
    ...
    def split(self, src, tgt):
        ...
        test_users = set(random.sample(co_users, round(self.ratio[1] * len(co_users))))     # not exactly cold-start
        train_src = src     # train data use all the sample in source domain
        train_tgt = tgt[tgt['uid'].isin(tgt_users - test_users)]
        test = tgt[tgt['uid'].isin(test_users)]
        # get the characteristic of co_users, add a new col to the train_meta and test.
    ...
    def main(self):
        ...
        src, tgt = self.mapper(src, tgt)    # map origin_id to number_id
